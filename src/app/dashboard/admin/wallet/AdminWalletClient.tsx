...
export function AdminWalletClient({ jobs }: { jobs: PaymentJob[] }) {
  const [filter, setFilter] = useState<'all' | 'paid' | 'unpaid'>('all')
  const [search, setSearch] = useState('')

  const paid   = jobs.filter(j => j.payment_status === 'paid')
  const unpaid = jobs.filter(j => j.payment_status !== 'paid' && j.driver_id)
  ...
  const filtered = jobs.filter(j => {
    const matchFilter =
      filter === 'all' ? true :
      filter === 'paid' ? j.payment_status === 'paid' :
      j.payment_status !== 'paid' && !!j.driver_id
  ...
  async function payDriver(jobId: string) {
    ...
    else alert(json.error ?? 'Failed to start payment')
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Driver payments for all completed jobs</p>
      </div>
        ...
        <div className="mb-5 flex items-start gap-3 bg-orange-50 border border-orange-200 rounded-xl px-4 py-3">
          ...
          <div>
            ...
            <p className="text-xs text-orange-600 mt-0.5">
              {formatCurrency(totalPending)} outstanding — pay drivers below to credit their wallets
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex-shrink-0 w-28 text-right">
                  {isPaid ? (
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors"
                    >
